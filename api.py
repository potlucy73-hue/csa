...
class BulkMCRequest(BaseModel):
    """Request model for bulk MC number extraction."""
    mc_numbers: List[str]

@app.post("/extract-via-github-action", response_model=BulkExtractResponse)
async def extract_via_github_action(request: BulkMCRequest):
    """
    Trigger extraction for a list of MC numbers.
    
    The extraction will run as a background process. No GitHub-specific information
    is exposed to the client.
    
    Args:
        request: BulkMCRequest containing list of MC numbers
    ...
    try:
        ...
        result = await github.trigger_workflow(mc_numbers)
        
        if result["status"] == "triggered":
            logger.info(f"GitHub Actions workflow triggered for {len(mc_numbers)} MC numbers")
            ...
            return BulkExtractResponse(
                ...
                total_mc_numbers=len(mc_numbers),
                status="processing",
                message="GitHub Actions workflow triggered. Check repository Actions tab for progress."
            )
        else:
            raise HTTPException(
                status_code=500,
